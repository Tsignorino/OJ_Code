...
void solve() {
    ...
    for (int i = 0; i < m; ++i) {
        auto p = max({
            pair(q[0].empty() ? INT_MIN / 2 : q[0].front(), 0), //
            pair(q[1].empty() ? INT_MIN / 2 : q[1].front(), 1), //
    ...
    for (int i = 1; i <= n + m; ++i) {
        pair<int, int> p = max({pair(q[0].empty() ? INT_MIN / 2 : q[0].front(), 0), pair(q[1].empty() ? INT_MIN / 2 : q[1].front(), 1), pair(q[2].empty() ? INT_MIN / 2 : q[2].front(), 2)});
